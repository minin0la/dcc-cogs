...
class Casino(Database, commands.Cog):
    ...
    @checks.admin_or_permissions(administrator=True)
    async def revokemem(
        self, ctx: commands.Context, player: Union[discord.Member, discord.User]
    ):
    ...
    async def stats(
        ...
        played = [
            y for x, y in sorted(player_data["Played"].items(), key=itemgetter(0))
        ]
        won = [y for x, y in sorted(player_data["Won"].items(), key=itemgetter(0))]
        cool_items = [
            y for x, y in sorted(player_data["Cooldowns"].items(), key=itemgetter(0))
        ]
    ...
    async def memdesigner(self, ctx: commands.Context):
        ...
        await ctx.send(
            _("Do you wish to `create`, `edit`, or `delete` an existing membership?")
        )
    ...
    async def global_casino_only(ctx):
        if await ctx.cog.config.Settings.Global() and not await ctx.bot.is_owner(
            ctx.author
        ):
            return False
        else:
    ...
    async def mode(self, ctx: commands.Context):
        ...
        if final.content.lower() == _("yes"):
            ...
            await ctx.send(
                _("Casino data deleted! Casino mode now set to {}.").format(alt)
            )
        else:
    ...
    async def _min(self, ctx: commands.Context, game: str, minimum: int):
        ...
        if minimum > games[game.title()]["Max"]:
            return await ctx.send(
                _("You can't set a minimum higher than the game's maximum bid.")
            )
    ...
    def parse_cooldowns(ctx, cooldowns, reduction):
        ...
        for cooldown in cooldowns:
            ...
            results.append(
                utils.cooldown_formatter(seconds, custom_msg="<<Ready to Play!")
            )
        return results
...
class Membership(Database):
    ...
    async def req_loop(self, membership):
        while True:
            ...
            await self.ctx.send(
                _("Would you like to continue adding or modifying requirements?")
            )
