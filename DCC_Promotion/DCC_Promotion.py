...
class DCC_PROMOTION(commands.Cog):
    ...
    async def promote(self, ctx, user: discord.Member, *, roleOrName: discord.Role):
        ...
        channel = discord.utils.get(
            ctx.guild.text_channels, name="evelyn-office"
        )  # Get evelyn-office channel
        ...
        bot_coders_channel = discord.utils.get(
            ctx.guild.text_channels, name="evelyn-logs"
        )
        old_role = user.top_role
        await ctx.message.delete()
        await user.add_roles(role_name)
        embed = discord.Embed(
            title="Downtown Cab Co. Management",
            colour=discord.Colour(0x7CFC00),
            description="Congratulations {},\n\nWe would like to inform you that you have been promoted from **{}** to **{}**\n\nKeep up the good work!".format(
                user.display_name, old_role.name, role_name.name
            ),
        )
        embed.set_thumbnail(
        ...
        try:
            ...
            await ctx.send(
                "**[Warning]** {} turned off their private messages".format(
                    user.mention
                )
            )
        await bot_coders_channel.send(
            content="{} uses command .promote to promote {} to rank {}".format(
                author.mention, user.mention, role_name.mention
            )
        )
    ...
    async def driveroftheweek(self, ctx, *, user: discord.Member):
        ...
        await role.edit(
            name="%s of the week" % user.top_role.name,
    ...
    async def demote(self, ctx, user: discord.Member, *, roleOrName: discord.Role):
        ...
        while user.top_role != role_name and user.top_role != nitroRole:
            try:
                ...
                await ctx.send(
                    "I don't have permissions to demote {}!".format(user.mention)
                )
        date = datetime.now().strftime("%Y-%m-%d")
        embed = discord.Embed(
            title="Downtown Cab Co. Management",
            colour=discord.Colour(0xFF0000),
            description="Dear {},\n\nThe Management has reviewed your case and based on the information and testimonials available, henceforth effective from {}, you are demoted from **{}** to **{}**.\n\nIf you have any question, contact the management.".format(
                user.display_name, date, old_role.name, role_name.name
            ),
        )
        embed.set_thumbnail(
            url="https://res.cloudinary.com/teepublic/image/private/s--4WWDcpP4--/t_Preview/b_rgb:ffb81c,c_limit,f_jpg,h_630,q_90,w_630/v1468933638/production/designs/84620_4.jpg"
        ...
        await bot_coders_channel.send(
            content="{} uses command .demote to demote {} to rank {}".format(
                author.mention, user.mention, role_name.mention
            )
        )
    ...
    async def dismiss(self, ctx, user: discord.Member):
        ...
        while user.top_role != server.default_role and user.top_role != nitroRole:
            try:
                ...
                await ctx.send(
                    "I don't have permissions to dismiss {}!".format(user.mention)
                )
        ...
        await management.send(
            content="{} uses command .dismiss to dismiss {}".format(
    ...
    async def suspend(self, ctx, user: discord.Member):
        ...
        while user.top_role != server.default_role and user.top_role != nitroRole:
            try:
                ...
                await ctx.send(
                    "I don't have permissions to suspend {}!".format(user.mention)
                )
        ...
        embed = discord.Embed(
            title="Downtown Cab Co. Management",
            colour=discord.Colour(0xFF0000),
            description="Dear {},\n\nThe Management has reviewed your case and based on the information and testimonials available, henceforth effective from {}, you are suspended until further notice.".format(
                user.display_name, date
            ),
        )
        embed.set_thumbnail(
        ...
        await management.send(
            content="{} uses command .suspend to suspend {}".format(
                author.mention, user.mention
            )
        )
        ...
        if role_name.name == "Trainee" or role_name.name == "Probationary Trainee":
            if any(r.name == "Interviewee" for r in user.roles):
                ...
                embed = discord.Embed(
                    title="Downtown Cab Co. Management",
                    colour=discord.Colour(0x7CFC00),
                    description="Dear {},\n\nCongratulation, you have been accepted into the team as Trainee. Read <#609560601009717278>, <#346665233491951617>, and <#543590685459087361>, if you have any out of character questions feel free to ask. Good luck!".format(
                        user.display_name, role_name.name
                    ),
                )
                if user.avatar.url == "":
                    embed.set_thumbnail(
                        url="https://images-ext-2.discordapp.net/external/k6AHx31fu7EiktzboQbX1BeOJ096MlVMKnXJawwYuF0/https/discordapp.com/api/guilds/301659110104104962/icons/71e58fd9b9c6a83b20f17b9eb8a671f8.jpg"
                    )
                else:
                ...
                await management.send(
                    content="{} uses command .accept to accept {} to the team with {} Roles".format(
                        author.mention, user.mention, role_name.mention
                    )
                )
            else:
            ...
            await ctx.send(
                """You cannot accept them in {}. You can only accept them in `Trainee` or `Probationary Trainee`""".format(
                    role_name.name
                )
            )
