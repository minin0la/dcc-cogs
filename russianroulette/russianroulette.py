    ...
    async def start_game(self, ctx):
        ...
        while len(filtered_players) > 1:
            await ctx.send(
                ...
                "place.*".format(counter, ctx.guild.me.display_name)
            )
    ...
    async def start_round(self, ctx, chamber, players):
        ...
        while True:
            for turn, player in enumerate(itertools.cycle(players), 1):
                await ctx.send(
                    "{} presses the revolver to their head and slowly squeezes the trigger...".format(player.display_name)
                )
                ...
                if turn == position:
                    ...
                    await ctx.send(msg.format(player.mention, random.choice(players).display_name, ctx.guild.owner))
                    await asyncio.sleep(3)
                    break
                else:
                    await ctx.send("**CLICK!** {} passes the gun along.".format(player.display_name))
                    await asyncio.sleep(3)
            break
